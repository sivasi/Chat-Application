	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0 ) {
			...
			if (!$result->num_rows > 0 ) {
				...
				$sql = "INSERT INTO users (name,username,email,password,created_at,verification_code,is_verified)
				VALUES ('$name','$username', '$email', '$password',CURRENT_TIMESTAMP(),'$v_code','0')";
				$result = mysqli_query($conn, $sql);
				if ($result && sendMail($_POST['email'],$v_code)){
					echo "<script>alert('Wow! User Registration Completed...please check Email id (spam folder) for email confirmation');
					...
					echo "<script>alert('Woops! Something Wrong Went. or server down..check your internet connection')</script>";
				}
			} 
			else {
